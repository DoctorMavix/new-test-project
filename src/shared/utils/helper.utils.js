...
module.exports = class HelperUtil {
    ...
    getFormatCount(count, codeSize, withLetter) {
        ...
        if (codeSize >= len) {
            ...
            if (withLetter) {
                ...
                const countNumber = this.countNumberInNumber(count, max, { fluor: true });
     ...
     */
    countNumberInNumber(number, divide, filter = { fluor: false }) {
        const { fluor } = filter;
    ...
    generateCodeFromModelCount(Model, filter) {
            const { codeSize, withLetter } = filter;
            let { modelSchema, addCount } = filter;
